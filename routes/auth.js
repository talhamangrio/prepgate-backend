...
router.post('/register', async (req, res) => {
  try {
    ...
    // Basic validation — missing fields would otherwise blow up bcrypt/JWT
    // further down and surface as an opaque 500.
    ...
    try {
      ...
      // Duplicate key error (E11000) — happens when two requests race past
      // the findOne check above at the same time (e.g. double-click submit,
      // or two users signing up within milliseconds of each other).
      // This is NOT a real server failure, just a "user already exists"
      // that slipped past the earlier check — handle it gracefully instead
      // of returning a scary 500.
      if (saveErr.code === 11000) {
        return res.status(400).json({ message: 'User already exists' });
      }
      throw saveErr; // anything else, let the outer catch log and handle it
    }
    ...
    console.error('❌ Register error:', err); // log the real cause instead of hiding it
...
router.post('/login', async (req, res) => {
  try {
    ...
    console.error('❌ Login error:', err); // log the real cause instead of hiding it
    res.status(500).json({ message: 'Server error' });
  }
});
